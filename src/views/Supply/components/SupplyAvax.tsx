...
import useSupplyToAvax from '../../../hooks/useSupplyToKeen';
import useRedeemFromAvax from '../../../hooks/useRedeemFromAvax';

const SupplyAvax: React.FC = () => {
  const keenFinance = useKeenFinance();
  const avaxStats = useAvaxStats();

  const [approveStatus, approve] = useApprove(keenFinance.AVAX, keenFinance.contracts.KeenRouter.address);
  const [approveStatusW, approveW] = useApprove(keenFinance.BKEENAVAX, keenFinance.contracts.KeenRouter.address);

  const tokenBalance = useTokenBalance(keenFinance.AVAX);
  //const stakedBalance = useStakedKeen();
  const stakedBalance = useTokenBalance(keenFinance.BKEEN_AVAX);
  ...
  const stakedTokenPriceInDollars = Number(avaxPriceInDollars);
  ...
  const { onStake } = useSupplyToAvax();
  const { onWithdraw } = useRedeemFromAvax();
        ...
        <CardContent>
          <StyledCardContentInner>
            <StyledCardHeader>
              ...
              <CardIcon>
                <TokenSymbol symbol="AVAX" />
              </CardIcon>

              <Button
                className={'shinyButton'}
                onClick={() => {
                  keenFinance.watchAssetInMetamask('AVAX');
                }}
                style={{
                  position: 'static',
                  top: '10px',
              ...
              <Label text={'bKEEN (AVAX)'} variant="yellow" />
